...
Each node has configurable fields set via `update_node_data` or in `build_graph` dataUpdates. Common fields:
...
- **Video AI**: `model`, `mode` (standard/pro), `duration` (5/10), `aspectRatio`

Use `get_node_type_info` for the exact field schema of any node — it returns all configurable fields with types, defaults, and allowed values.
